...
class SubtopicConfig(pydantic.BaseModel):
    field_id: Union[int, str]
    type: Literal['bool', 'bool[]', 'float', 'float[]', 'int', 'int[]', 'str']


class TopicConfig(pydantic.BaseModel):
    connection_type: Literal['tcp', 'udp']
    port: int

    parsing_strategy: Literal['delimited', 'json_array', 'json_dict', 'raw']
    delimiter: Optional[str] = None
    use_regex_delimiter: bool = False

    subtopics: Dict[str, SubtopicConfig] = pydantic.Field(default_factory=dict)

    @pydantic.model_validator(mode='after')
    def check_delimiter(self) -> Self:
        if self.parsing_strategy == 'delimited' and self.delimiter is None:
            raise ValueError('delimiter is required for delimited parsing '
                             'strategy')

        if self.parsing_strategy != 'raw' and not self.subtopics:
            raise ValueError(f'subtopics required for {self.parsing_strategy} '
                             'parsing strategy')

        if self.parsing_strategy == 'delimited' and self.subtopics:
            if any(sub.type.endswith('[]') for sub in self.subtopics.values()):
                raise ValueError('Arrays are not a supported field type for '
...
assert set(ROS_TYPE_MAP.keys()) == \
       set(typing.get_args(typing.get_type_hints(SubtopicConfig)['type']))
...
def extract_messages(buffer: bytes, strategy: str, config: TopicConfig) \
    ...
    Args:
        ...
        config: TopicConfig containing strategy-specific settings
    ...
    if strategy == "raw":
        ...
        delimiter = typing.cast(str, config.delimiter).encode()
        use_regex = config.use_regex_delimiter
        if not use_regex:
...
def parse_message(message: bytes, strategy: str, config: TopicConfig) -> dict:
...
def parse_delimited_message(message: bytes, config: TopicConfig) -> dict:
    """Parse delimited message into fields

    Args:
        message: Raw message bytes
        config: TopicConfig containing delimiter and subtopic definitions
        ...
        for subtopic_name, subtopic_config in config.subtopics.items():
            ...
            try:
                if subtopic_config.type == "float":
                    ...
                    raise ParsingError("Unsupported delimited field type: "
                                       f"{subtopic_config.type}")

                result[subtopic_name] = value

            except ValueError as e:
                raise ParsingError(f"Error converting field {field_id} to "
                                   f"{subtopic_config.type}: {str(e)}")
...
def parse_json_dict_message(message: bytes, config: TopicConfig) -> dict:
    """Parse JSON dictionary message

    Args:
        message: Raw message bytes
        config: TopicConfig containing subtopic definitions
    ...
    try:
        ...
        # Process each subtopic
        for subtopic_name, subtopic_config in config.subtopics.items():
            field_id = subtopic_config.field_id  # will be JSON dict key
            ...
            if not is_valid:
                raise ParsingError(f"Field '{field_id}' is not a valid "
                                   f"{subtopic_config.type}")
...
def parse_json_array_message(message: bytes, config: TopicConfig) -> dict:
...
def setup_publishers(config: ConfigModel) -> Dict[str, dict]:
    """Set up publishers for all topics and subtopics

    Args:
        config: ConfigModel containing complete node configuration
...
def publish_messages(topic_name: str, parsed_data: dict, publishers: dict,
                    parsing_strategy: str, topic_config: TopicConfig) -> None:
    ...
    Args:
        ...
        topic_config: TopicConfig of configuration values
    ...
    if parsing_strategy == "raw":
        ...
        for subtopic_name, value in parsed_data.items():
            sub_config = topic_config.subtopics[subtopic_name]
            msg = convert_to_ros_msg(value, sub_config.type)
...
def main():
    ...
    # Load and validate configuration via Pydantic schema
    raw_cfg = rospy.get_param('~', None)
    if raw_cfg is None:
        rospy.logwarn("No network_data_capture configuration found, stopping node")
        exit(0)

    try:
        config = ConfigModel.model_validate(raw_cfg)
    ...
    for topic_name, topic_config in config.topics.items():
        try:
            sock = create_socket(topic_config.connection_type,
                                 topic_config.port)
            sockets[topic_name] = sock
            buffers[topic_name] = b""
            stats[topic_name] = TopicStats()

            rospy.loginfo(f"Created {topic_config.connection_type.upper()} "
                          f"socket for {topic_name} "
